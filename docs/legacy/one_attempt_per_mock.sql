-- ============================================================
-- OBSOLETE — DO NOT APPLY. Kept for historical record only.
-- ------------------------------------------------------------
-- This file documented a "one attempt per mock, ever" rule that
-- has since been DELIBERATELY REVERSED. The unique constraint it
-- describes (mock_test_results_user_mock_unique) has been DROPPED,
-- and get_mock_access() no longer checks mock_test_results at all.
--
-- The current, correct rule (as of the session that reversed this)
-- is: unlimited completed attempts of the same mock are allowed,
-- for both Pass users (free) and credit users (1 credit per
-- attempt/re-attempt). See mock_test_sessions, start_or_resume_
-- mock_test(), and start_reattempt() for the current architecture —
-- attempt history/limits now live there, not on mock_test_results.
--
-- The original file content follows, unmodified, for reference only.
-- ============================================================
