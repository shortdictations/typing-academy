...
/* ============================================================
   ...
   ------------------------------------------------------------
   Shared across every page that can call start_or_resume_mock_test()
   or start_reattempt() and get back is_resumed=true (dashboard.js,
   mock-test.js, mock-history.js) — replaces a bare alert() with the
   actual unfinished mock's details and an explicit choice, matching
   the supplied reference design. Injects its own markup into the
   page on first call (nothing needs to pre-exist in any page's own
   HTML). Returns a Promise<boolean>: true if the student clicked
...
function showUnfinishedTestModal({ mockTitle, category, duration, startedAt }) {
    ...
    modal.innerHTML = `
      <div class="utc-card">
        ...
        <div class="utc-info-card">
          ...
          <div>
            <div class="utc-info-title" id="utcMockTitle"></div>
            <div class="utc-info-meta">
              <span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg><span id="utcDuration"></span></span>
              <span class="utc-info-sep">|</span>
              <span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg><span id="utcStartedAt"></span></span>
            </div>
          </div>
        </div>
  ...
  modal.hidden = false;
  requestAnimationFrame(() => requestAnimationFrame(() => modal.classList.add("utc-anim-in-active")));

  return new Promise(resolve => {
    function close(result) {
      modal.classList.remove("utc-anim-in-active");
    ...
    const cancelBtn = document.getElementById("utcCancelBtn");
    const continueBtn = document.getElementById("utcContinueBtn");
    cancelBtn.addEventListener("click", onCancel);
    continueBtn.addEventListener("click", onContinue);
  });
}

// "just now" / "5 minutes ago" / "2 hours ago" / "3 days ago" — kept
// deliberately coarse (no seconds granularity) since exact precision
// doesn't matter for this message, just a rough sense of how long the
// test has been sitting unfinished.
function formatRelativeStartTime(isoString) {
